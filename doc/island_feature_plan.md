...
run()
  ...
  │     ├── 驳回次数 < 10：
  │     │     ├── 检测三个货物格子的图标（直接模板比对）
  │     │     │     ├── 检测是否有芝士（TEMPLATE_CHEESE）
  │     │     │     └── 检测是否有豆腐（TEMPLATE_TOFU）
  │     │     │
  │     │     ├── 有芝士 或 有豆腐 → 点击左侧"驳回"按钮
  │     │     │                       记录该订单区域到"跳过列表"
  │     │     │                       驳回次数 +1，回到初始逻辑
  │     │     │
  │     │     └── 无芝士且无豆腐 → 点击"交付"按钮
...
每个订单最多 3 个物品（竖向排列）：
  ├── 第 1 个物品：必定有内容
  │     ├── 检测是否为"芝士"模板（TEMPLATE_CHEESE）
  │     └── 是芝士 → 标记需驳回
  │
  ├── 第 2 个物品：检测区域是否与"默认空白图"相同
  │     ├── 空白 → 跳过，第 3 个也不用检查
  │     └── 有内容 → 检测是否为芝士
  │
  └── 第 3 个物品：检测区域是否与"默认空白图"相同
        ├── 空白 → 跳过
        └── 有内容 → 检测是否为芝士
```
...
驳回后更新 `UrgentDetectRefreshTime` 的计算方式，上限 **1 小时**：

```python
from datetime import datetime, timedelta

def calculate_refresh_time(reject_count):
    """
    根据当前驳回次数计算紧急委托检测刷新时间。
    上限 1 小时。
    
    公式：
      delay_minutes = min(60, max(10, reject_count * 10))
      delay_seconds = delay_minutes * 60 + 10  # 额外加 10 秒防网络波动
    
    Returns:
        datetime: 下次可检测紧急委托的时间点
    """
    delay_minutes = min(60, max(10, reject_count * 10))  # 前10次固定10分钟，上限60分钟
    delay_seconds = delay_minutes * 60 + 10               # 加 10 秒网络缓冲
...
- 点击紧急委托后发现交付按钮非蓝色 → 记录当前时间作为刷新时间
- 在刷新时间到期前：跳过紧急委托检测，只检测挑战/轻松图标
- 刷新时间到期后：恢复正常检测（包括紧急委托）

### 新增资产（assets）需求

| 资产 | 类型 | 说明 |
|------|------|------|
| `DAILY_ORDER_URGENT` | Button | 紧急委托图标按钮 |
| `DAILY_ORDER_CHALLENGE` | Button | 挑战类型图标按钮 |
| `DAILY_ORDER_EASY` | Button | 轻松类型图标按钮 |
| `DAILY_ORDER_PREPARING` | Button | 准备中图标按钮 |
| `DAILY_ORDER_DELIVER` | Button | 交付按钮（检测蓝色启用状态） |
| `DAILY_ORDER_REJECT` | Button | 驳回按钮（左侧） |
| `DAILY_ORDER_CHECK` | Button | 每日订单界面检测图片 |
| `TEMPLATE_CHEESE` | Template | 芝士物品模板（直接比对三个格子） |
| `TEMPLATE_TOFU` | Template | 豆腐物品模板（直接比对三个格子） |
| `POPUP_RESOURCE_INSUFFICIENT` | Button | 资源不足弹窗检测（点击关闭） |
| `ISLAND_BACK` | Button | 返回按钮（复用已有资产，`(6, 21, 99, 66)`） |
...
IslandDailyOrder:
  Enabled: true                            # 启用每日订单
                                           # help: 不支持"标准"类型每日订单。
  RejectCount: 0                           # 当前驳回次数（可手动修改，每日 0:00 自动重置）
  UrgentDetectRefreshTime: "2020-01-01 00:00:00"  # 紧急委托检测刷新时间
  RejectFilter: "Cheese > Tofu"            # 驳回物品过滤
                                           # help: 仅支持 Cheese（芝士）和 Tofu（豆腐）。
```
